...
const Slider = () => {
  ...
  const imgSliderFuncPiu = () => {
    if (imgSlider === imgArray.length - 1) {
      setImgSlider(imgSlider);
    } else {
      setImgSlider(imgSlider + 1);
    }
  };

  const imgSliderFuncMeno = () => {
    if (imgSlider === 0) {
      setImgSlider(imgSlider);
    } else {
      setImgSlider(imgSlider - 1);
  ...
  return (
    <div className="Slider">
      <img src={imgArray[imgSlider]} alt="img" />
      <div className="btn-slider">
        <button onClick={imgSliderFuncMeno}>{"<"}</button>
        <button onClick={imgSliderFuncPiu}>{">"}</button>
      </div>
    </div>
  );
};
